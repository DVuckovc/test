...
/*-----------------------------------------------------------*/
/* hooks required by the FreeRTOS */
...
int main( void )
{
	...
	/* start tacking the system operation */
	#if (configUSE_TRACE_FACILITY == 1)
		vTraceEnable(TRC_START);

	#endif

	/* lets make some tasks in heap */
	portBASE_TYPE xReturned;
	xReturned = xTaskCreate (vTask1, "Task1",240,NULL,1,NULL);
	
	xReturned = xTaskCreate (vTask2, "Task2",240,NULL,1,NULL);

	/* Tasks created lets start the scheduler */
	vTaskStartScheduler();

	/* this is a threaded application so we should never get here */
	while(1){};
